...
<tr p-id='${datos.id}'>
    ...
    <td>
        <div class="d-flex">
            <button class="btn btn-warning btn-sm mb-1">
                <a href="{{url('/forma_pago/')}}/${datos.id}/edit">
                    <img class="icon" src="{{asset('img/nuevoEditar.svg')}}" alt="" srcset="">
                </a>
            </button>
            <p></p>
            <form action="{{ url('/forma_pago/') }}/${datos.id}" method="post">
                @csrf
                {{method_field('DELETE')}}
                <button type="submit" onclick="return confirm('Seguro:¿Qué quieres eliminar esta forma de pago?')"
                    class="btn btn-danger btn-sm">
                    <img class="icon" src="{{asset('img/nuevoEliminar.svg')}}" alt="" srcset="">
                </button>
