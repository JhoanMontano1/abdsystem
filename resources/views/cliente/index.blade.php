...
@if(Session::has('mensaje'))
   ...
   <form action="{{url('/report')}}" method="get" class="btn btn-primary">
        <input type="hidden" name="context" value="cliente">
    ...
    <br>
    <br>
    <form action="{{url('/searchCli')}}" method="get">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <form action="{{ url('/cliente/'.$clientes->id) }}" method="post">
                            @csrf
                            {{method_field('DELETE')}}
                            <button type="submit" onclick="return confirm('Seguro,¿Qué quieres eliminar a este cliente?')"
